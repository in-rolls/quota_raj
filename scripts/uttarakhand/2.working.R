...
data_2008 <- filter(ukhand_data, year == 2008)
data_2010$key <- paste0(ukhand_data$year, ukhand_data$elected_position, ukhand_data$district,
                        ukhand_data$declared_result, ukhand_data$unopposed_winner, ukhand_data$development_block
                        
                        unique_df <- distinct(ukhand_data, year, elected_position, district,
                                              declared_result, unopposed_winner, development_block, 
                                              gram_panchayat, reservation_status, winner_name, 
                                              election_symbol, received_votes, runner_up_name,
                                              election_symbol_1, secured_votes, vote_difference, district_panchayat_ward, zilla_panchayat_ward, serial_number, candidate_name, father_husband_name)
                        
                        paste(colnames(ukhand_data), collapse = ", ")
                        collapse = 
