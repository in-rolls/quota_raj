...
uk_all <- read_parquet("/Users/varun/Library/CloudStorage/Dropbox/USC/quota_raj/data/uttarakhand/uk_all_fuzzy.parquet")
...
uk_all <- read_parquet("data/up/uk_all_fuzzy.parquet")
...
uk_all <- uk_all %>%
     mutate(
          ...
          obc_2005 = ifelse(uk_all$gp_res_status_fin_eng_2005 %in% c("Other Backward Class - Female"), 1, 0),
          obc_2010 = ifelse(uk_all$gp_res_status_fin_eng_2010 %in% c("Other Backward Class - Female"), 1, 0),
          obc_2015 = ifelse(uk_all$gp_reservation_status_eng_2015 %in% c("Other Backward Class Female"), 1, 0),
          obc_2021 = ifelse(uk_all$gp_reservation_status_eng_2021 %in% c("Other Backward Class Female"), 1, 0),
          
          dalit_2005 = ifelse(uk_all$gp_res_status_fin_eng_2005 %in% c("Scheduled Caste - Female", "Scheduled Tribe - Female"), 1, 0),
          dalit_2010 = ifelse(uk_all$gp_res_status_fin_eng_2010 %in% c("Scheduled Caste - Female", "Scheduled Tribe - Female"), 1, 0),
          dalit_2015 = ifelse(uk_all$gp_reservation_status_eng_2015 %in% c("Scheduled Caste - Female", "Scheduled Tribe - Female"), 1, 0),
          dalit_2020 = ifelse(uk_all$gp_reservation_status_eng_2021 %in% c("Scheduled Caste Female", "Scheduled Tribe Female"), 1, 0),
...
save(uk_all, file = "data/up/uk_all_fuzzy_recoded.RData")
