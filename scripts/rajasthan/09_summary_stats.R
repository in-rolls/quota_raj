...
# Close Election Formal Analysis ------------------------------------------
...
test <- contest_sarpanch %>% 
     filter(key_2020=="udaipurvallabhnagar panchayat samitiadindavardi")

test1 <- winner_sarpanch %>% 
     filter(key_2020=="udaipurvallabhnagar panchayat samitiadindavardi")


test <- contest_sarpanch %>% select(key_2020)
test1 <- winner_sarpanch %>% select(key_2020)


winner_sarpanch <- winner_sarpanch %>%
     left_join(contest_sarpanch %>%
                    select(key_2020, nameofcontestingcandidate, gender) %>%
                    rename(winner_gender = gender),
               by = c("key_2020", "winnercandidatename" = "nameofcontestingcandidate"))
