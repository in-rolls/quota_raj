...
up_05_10 <- filter_key_collisions(up_05_10, "2005_2010") %>% mutate(gp_id = key_2010)
...
up_10_15 <- filter_key_collisions(up_10_15, "2010_2015") %>% mutate(gp_id = key_2010)
...
up_15_21 <- inner_join(up_2015_dedupe, up_2021_dedupe, by = c("match_key_2015" = "match_key_2021")) %>%
  ...
  mutate(match_key_native = match_key, gp_id = match_key)
...
history_ambiguous <- up_all_fuzzy %>%
  add_count(key_2010, name = "history_links") %>%
  filter(is.na(key_2010) | history_links > 1L)
write_csv(
  history_ambiguous %>% select(any_of(c("key_2005", "key_2010", "key_2015", "key_2021", "match_key", "history_links"))),
  here("data/crosswalks/audit/02b_up_ambiguous_histories.csv")
)
up_05_21 <- up_all_fuzzy %>%
  group_by(key_2010) %>%
  filter(!is.na(key_2010), n() == 1L) %>%
  ungroup() %>%
  mutate(gp_id = key_2010) %>%
  mutate(
    ...
    obc_2005 = as.integer(grepl("Other Backward Class", gp_res_status_fin_eng_2005)),
    obc_2010 = as.integer(grepl("Other Backward Class", gp_res_status_fin_eng_2010)),
    obc_2015 = as.integer(grepl("Other Backward Class", gp_reservation_status_eng_2015)),
    obc_2021 = as.integer(grepl("Other Backward Class", gp_reservation_status_eng_2021)),
    dalit_2005 = as.integer(grepl("Scheduled Caste|Scheduled Tribe", gp_res_status_fin_eng_2005)),
    dalit_2010 = as.integer(grepl("Scheduled Caste|Scheduled Tribe", gp_res_status_fin_eng_2010)),
    dalit_2015 = as.integer(grepl("Scheduled Caste|Scheduled Tribe", gp_reservation_status_eng_2015)),
    dalit_2021 = as.integer(grepl("Scheduled Caste|Scheduled Tribe", gp_reservation_status_eng_2021)),
