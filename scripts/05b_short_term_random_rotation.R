# 05b_short_term_random_rotation.R
...
message("=== Short-Term Effects: Random Rotation Districts ===")
...
# Rajasthan panels
chisq_raj_0510 <- compute_district_chisq(raj_05_10, "treat_2005", "treat_2010", "district_std_2010")
chisq_raj_1015 <- compute_district_chisq(raj_10_15, "treat_2010", "treat_2015", "district_std_2015")
chisq_raj_1520 <- compute_district_chisq(raj_15_20, "treat_2015", "treat_2020", "district_std_2020")
...
raj_05_10_rr <- raj_05_10 %>%
    filter(district_std_2010 %in% random_raj_0510, treat_2010 == 0)
message("Open seats in random-rotation districts: ", nrow(raj_05_10_rr))
...
raj_10_15_rr <- raj_10_15 %>%
    filter(district_std_2015 %in% random_raj_1015, treat_2015 == 0, !is.na(female_winner_2015))
message("Open seats in random-rotation districts: ", nrow(raj_10_15_rr))
...
raj_15_20_rr <- raj_15_20 %>%
    filter(district_std_2020 %in% random_raj_1520, treat_2020 == 0, !is.na(female_winner_2020))
message("Open seats in random-rotation districts: ", nrow(raj_15_20_rr))
...
message("Open seats in random-rotation districts: ", nrow(up_05_10_rr))
...
message("Open seats in random-rotation districts: ", nrow(up_10_15_rr))
...
message("Open seats in random-rotation districts: ", nrow(up_15_21_rr))
...
if (length(valid_panels) == 0) {
    message("ERROR: No models could be estimated. Check data availability.")
    quit(status = 1)
}

message("Panels with valid models: ", paste(names(valid_panels), collapse = ", "))
if (length(missing_panels) > 0) {
    message("Panels without valid models (will show placeholders): ", paste(names(missing_panels), collapse = ", "))
}
...
# Extract single-variable model stats using broom

get_single_var_stats <- function(m) {
    if (is.null(m)) return(NULL)
    stats <- tidy(m) %>% filter(grepl("^treat_", term))
    model_info <- glance(m)
    treat_row <- stats[1, ]
    list(
        coef = treat_row$estimate,
        se = treat_row$std.error,
        pval = treat_row$p.value,
...
stats_raj_0510_nofe <- get_single_var_stats(m_raj_0510_nofe)
stats_raj_0510_fe <- get_single_var_stats(m_raj_0510_fe)
stats_raj_1015_nofe <- get_single_var_stats(m_raj_1015_nofe)
stats_raj_1015_fe <- get_single_var_stats(m_raj_1015_fe)
stats_raj_1520_nofe <- get_single_var_stats(m_raj_1520_nofe)
stats_raj_1520_fe <- get_single_var_stats(m_raj_1520_fe)
stats_up_0510_nofe <- get_single_var_stats(m_up_0510_nofe)
stats_up_0510_fe <- get_single_var_stats(m_up_0510_fe)
stats_up_1015_nofe <- get_single_var_stats(m_up_1015_nofe)
stats_up_1015_fe <- get_single_var_stats(m_up_1015_fe)
stats_up_1521_nofe <- get_single_var_stats(m_up_1521_nofe)
stats_up_1521_fe <- get_single_var_stats(m_up_1521_fe)
...
coef_row <- paste0(
    "   $\\text{Quota}_{t-1}$             & ",
    format_coef_stars(stats_raj_0510_nofe$coef, stats_raj_0510_nofe$pval), "    & ",
    format_coef_stars(stats_raj_0510_fe$coef, stats_raj_0510_fe$pval), "          & ",
    format_coef_stars(stats_raj_1015_nofe$coef, stats_raj_1015_nofe$pval), " & ",
    format_coef_stars(stats_raj_1015_fe$coef, stats_raj_1015_fe$pval), "  & ",
    format_coef_stars(stats_raj_1520_nofe$coef, stats_raj_1520_nofe$pval), "   & ",
    format_coef_stars(stats_raj_1520_fe$coef, stats_raj_1520_fe$pval), "          & ",
    format_coef_stars(stats_up_0510_nofe$coef, stats_up_0510_nofe$pval), " & ",
    format_coef_stars(stats_up_0510_fe$coef, stats_up_0510_fe$pval), "  & ",
    format_coef_stars(stats_up_1015_nofe$coef, stats_up_1015_nofe$pval), "              & ",
    format_coef_stars(stats_up_1015_fe$coef, stats_up_1015_fe$pval), " & ",
    format_coef_stars(stats_up_1521_nofe$coef, stats_up_1521_nofe$pval), "        & ",
    format_coef_stars(stats_up_1521_fe$coef, stats_up_1521_fe$pval), "\\\\  "
)

# SE row
se_row <- paste0(
    "                                    & ",
    format_se_parens(stats_raj_0510_nofe$se), " & ",
    format_se_parens(stats_raj_0510_fe$se), "       & ",
    format_se_parens(stats_raj_1015_nofe$se), " & ",
    format_se_parens(stats_raj_1015_fe$se), "        & ",
    format_se_parens(stats_raj_1520_nofe$se), " & ",
    format_se_parens(stats_raj_1520_fe$se), "        & ",
    format_se_parens(stats_up_0510_nofe$se), "      & ",
    format_se_parens(stats_up_0510_fe$se), "       & ",
    format_se_parens(stats_up_1015_nofe$se), "               & ",
    format_se_parens(stats_up_1015_fe$se), " & ",
    format_se_parens(stats_up_1521_nofe$se), "        & ",
    format_se_parens(stats_up_1521_fe$se), "\\\\  "
)

# R2 row
r2_row <- paste0(
    "   R$^2$                            & ",
    format_r2_val(stats_raj_0510_nofe$r2), "  & ",
    format_r2_val(stats_raj_0510_fe$r2), "          & ",
    format_r2_val(stats_raj_1015_nofe$r2), "  & ",
    format_r2_val(stats_raj_1015_fe$r2), "          & ",
    format_r2_val(stats_raj_1520_nofe$r2), "  & ",
    format_r2_val(stats_raj_1520_fe$r2), "          & ",
    format_r2_val(stats_up_0510_nofe$r2), "       & ",
    format_r2_val(stats_up_0510_fe$r2), "          & ",
    format_r2_val(stats_up_1015_nofe$r2), "  & ",
    format_r2_val(stats_up_1015_fe$r2), " & ",
    format_r2_val(stats_up_1521_nofe$r2), "        & ",
    format_r2_val(stats_up_1521_fe$r2), "\\\\  "
)

# Observations row
n_row <- paste0(
    "   Observations                     & ",
    format_n_comma(stats_raj_0510_nofe$n), "   & ",
    format_n_comma(stats_raj_0510_fe$n), "         & ",
    format_n_comma(stats_raj_1015_nofe$n), "    & ",
    format_n_comma(stats_raj_1015_fe$n), "           & ",
    format_n_comma(stats_raj_1520_nofe$n), "  & ",
    format_n_comma(stats_raj_1520_fe$n), "         & ",
    format_n_comma(stats_up_0510_nofe$n), "       & ",
    format_n_comma(stats_up_0510_fe$n), "        & ",
    format_n_comma(stats_up_1015_nofe$n), "                & ",
    format_n_comma(stats_up_1015_fe$n), " & ",
    format_n_comma(stats_up_1521_nofe$n), "        & ",
    format_n_comma(stats_up_1521_fe$n), "\\\\  "
)
...
message("Created: tabs/short_term_random_rotation.tex")
