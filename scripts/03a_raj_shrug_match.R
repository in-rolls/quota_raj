...
missing_samitis <- elex_samitis %>%
  anti_join(
    ...
    by = c(
      "dist_lower" = "elex_district_lower",
...
elex_with_block <- elex %>%
  ...
  left_join(
    ...
    by = c(
      "dist_name_lower" = "elex_district_lower",
...
message(
  "Election rows with block code: ", sum(!is.na(elex_with_block$lgd_block_code)),
...
shrug_lgd <- read_csv(here("data/shrug_gp_xwalk/data/shrug_LGD_matched.csv"), show_col_types = FALSE) %>%
  ...
  summarize(
    shrid2 = min(shrid2),
    local_body_name = min(local_body_name),
    n_villages = n(),
