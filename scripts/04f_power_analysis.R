...
message("=== Power Analysis and Minimum Detectable Effects ===")
...
data_05_10 <- raj_05_10 %>%
    filter(treat_2010 == 0 & !is.na(female_winner_2010))
...
message(sprintf("Raj 05-10: N=%d, Coef=%.4f, SE=%.4f\n", n_05_10, coef_05_10, se_05_10))
...
if (treat_2010_var) {
    ...
    message(sprintf("Raj 10-15: N=%d, Coef=%.4f, SE=%.4f\n", n_10_15, coef_10_15, se_10_15))
    include_raj_10_15 <- TRUE
} else {
    message("Raj 10-15: Skipped - no variation in treatment variable")
...
message(sprintf("Raj 15-20: N=%d, Coef=%.4f, SE=%.4f\n", n_15_20, coef_15_20, se_15_20))
...
message(sprintf("UP 05-10: N=%d, Coef=%.4f, SE=%.4f\n", n_up_05_10, coef_up_05_10, se_up_05_10))
...
message(sprintf("UP 10-15: N=%d, Coef=%.4f, SE=%.4f\n", n_up_10_15, coef_up_10_15, se_up_10_15))
...
message(sprintf("UP 15-21: N=%d, Coef=%.4f, SE=%.4f\n", n_up_15_21, coef_up_15_21, se_up_15_21))
...
message(sprintf("Rajasthan LT: N=%d, Control=%.3f, Coef=%.4f, SE=%.4f, MDE=%.4f\n",
...
message(sprintf("UP LT: N=%d, Control=%.3f, Coef=%.4f, SE=%.4f, MDE=%.4f\n",
...
for (i in 1:nrow(power_df)) {
    message(sprintf("   %s %s: MDE = %.1f pp (%.0f%% of control mean %.1f%%)\n",
...
for (i in 1:nrow(power_df)) {
    message(sprintf("   %s %s: Can rule out effects > %.1f pp (%.0f%% increase from %.1f%%)\n",
                power_df$State[i], power_df$Period[i],
...
message("   Bhavnani found ~15pp effect (5x increase from ~3% baseline)")
message("   Our precision allows detecting effects as small as:")
raj_st <- power_df %>% filter(State == "Rajasthan", Type == "Short-term")
up_st <- power_df %>% filter(State == "UP", Type == "Short-term")
message(sprintf("   - Rajasthan: %.1f-%.1f pp\n", min(raj_st$MDE_80)*100, max(raj_st$MDE_80)*100))
message(sprintf("   - UP: %.1f-%.1f pp\n", min(up_st$MDE_80)*100, max(up_st$MDE_80)*100))
message("   We would detect a Bhavnani-sized effect with >99% power.")
...
message("Saved: figs/mde_plot.pdf")
...
message(sprintf("  Rajasthan short-term pooled MDE: %.2f pp\n", raj_st_pooled_mde * 100))

up_st_pooled_se <- 1/sqrt(1/se_up_05_10^2 + 1/se_up_10_15^2 + 1/se_up_15_21^2)
up_st_pooled_mde <- calc_mde(up_st_pooled_se)
message(sprintf("  UP short-term pooled MDE: %.2f pp\n", up_st_pooled_mde * 100))

message("\nLong-term:")
message(sprintf("  Rajasthan long-term MDE: %.2f pp\n", mde_raj_lt * 100))
message(sprintf("  UP long-term MDE: %.2f pp\n", mde_up_lt * 100))
...
message(sprintf("  Overall pooled MDE: %.2f pp\n", overall_mde * 100))
