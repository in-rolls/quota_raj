...
contest$key_2020 <- paste0(
    gsub(" ", "", contest$district),
...
contest_prop <- contest %>%
    select(key_2020, prop_women, women_present, num_women, total_contestants) %>%
    distinct()
raj_panch <- raj_panch %>% left_join(contest_prop, by = "key_2020")
...
contest_sarpanch <- contest_sarpanch %>%
    ...
    filter(electiontype != "by election") %>%
    mutate(
        key_2020 = paste0(district, panchayatsamiti, nameofgrampanchayat),
...
winner_sarpanch$new_key <- paste0(
    gsub(" ", "", winner_sarpanch$district),
...
cat("Saved consolidated candidacy table to tabs/raj_candidacy_combined.tex\n")
