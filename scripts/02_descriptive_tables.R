...
cat("Generated tabs/data_sources.tex, tabs/treat_status.tex, tabs/gp_summary.tex, and tabs/state_local_totals.tex\n")
...
if (!is.null(india_states)) {
    ...
    cat("Saved: figs/india_map_up_raj.pdf\n")
} else {
    cat("\nWARNING: India shapefile data not available.\n")
    cat("To generate the map, install rnaturalearth:\n")
    cat("  install.packages('rnaturalearth')\n")
    cat("  install.packages('rnaturalearthdata')\n")
}
