...
cat("=== Balance Tables with F-stats and Randomization Inference ===\n")
...
compute_balance_stats <- function(data, treat_y1, treat_y2, panel_name) {
    ...
    cat("Sample N:", nrow(data), "\n")

    if (nrow(data) < 100) {
        cat("WARNING: Sample too small for reliable inference\n")
        return(NULL)
    }

    # Get available balance variables
    avail_vars <- intersect(names(balance_vars), names(data))
    cat("Available balance variables:", length(avail_vars), "\n")

    if (length(avail_vars) == 0) {
        cat("ERROR: No balance variables found in data\n")
        return(NULL)
    }
    ...
    if (!is.na(fstat)) {
        ...
        cat("F-stat (LR):", round(fstat, 2), ", df:", df, ", p:", round(p_fstat, 4), "\n")
    } else {
        p_fstat <- NA
        cat("F-stat computation failed\n")
    }

    # Randomization Inference p-value
    cat("Computing RI p-value (1000 permutations)...\n")
    p_ri <- compute_ri_pvalue(data_complete, treat_y1, avail_vars, n_perm = 1000)
    cat("RI p-value:", round(p_ri, 4), "\n")
...
if (!is.null(raj_stats_0510) && !is.null(raj_stats_1015) && !is.null(raj_stats_1520)) {
    ...
    combined_tex <- c(combined_tex,
        ...
        paste0("Joint p (LR) & \\multicolumn{3}{c}{", fmt(raj_stats_0510$p_fstat, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(raj_stats_1015$p_fstat, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(raj_stats_1520$p_fstat, 3), "} \\\\"),
        paste0("Joint p (RI) & \\multicolumn{3}{c}{", fmt(raj_stats_0510$p_ri, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(raj_stats_1015$p_ri, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(raj_stats_1520$p_ri, 3), "} \\\\"),
        "\\bottomrule",
        "\\end{tabular}",
        "\\par}",
        "",
    ...
    cat("Created: tabs/balance_raj.tex\n")
}
...
if (!is.null(up_stats_0510) && !is.null(up_stats_1015) && !is.null(up_stats_1521)) {
    ...
    combined_up_tex <- c(combined_up_tex,
        ...
        paste0("Joint p (LR) & \\multicolumn{3}{c}{", fmt(up_stats_0510$p_fstat, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(up_stats_1015$p_fstat, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(up_stats_1521$p_fstat, 3), "} \\\\"),
        paste0("Joint p (RI) & \\multicolumn{3}{c}{", fmt(up_stats_0510$p_ri, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(up_stats_1015$p_ri, 3), "} & ",
               "\\multicolumn{3}{c}{", fmt(up_stats_1521$p_ri, 3), "} \\\\"),
        "\\bottomrule",
        "\\end{tabular}",
        "\\par}",
        "",
    ...
    cat("Created: tabs/balance_up.tex\n")
}
